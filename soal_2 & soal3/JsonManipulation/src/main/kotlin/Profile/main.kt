...
fun main() {
    ...
    var dataProfile: List<Profile> = gson.fromJson(json, listType)
    ...
    for (user in dataProfile.indices) {
        when (input) {
            ...
            3 -> {
                ...
                if (splitName != null) {
                    for (word in 0..splitName.size - 1) {
                        var splitWord = splitName[word].split("")
                        var name = ""
            ...
            4 -> {
                val articles = dataProfile[user].articles?.size
                if (articles != 0) {
                    var publish = dataProfile[user].articles
            ...
            6 -> {
                ...
                if (totalArticles != 0) {
                    for (count in articles.indices) {
                        ...
                        var splitName = nameArticles.split(" ")
                        for (word in 0..splitName.size - 1) {
                            var splitWord = splitName[word].split("")
                            var wordTips = ""
                            for (char in 1..splitWord.size-1){
